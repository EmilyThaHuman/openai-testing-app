...
// Initialize the encoder with the base configuration and special tokens.
...
// Encodes chat messages based on the model options.
export const getChatGPTEncoding = (messages, model) => {
...
// Counts the tokens in a message array.
const countTokens = (messages, model) => {
  if (messages.length === 0) return 0;
  return getChatGPTEncoding(messages, model).length;
};

// Limits messages by the specified token limit.
export const limitMessageTokens = (messages, limit = 4096, model) => {
...
// Updates the total token usage in the application state.
export const updateTotalTokenUsed = (
  ...
  const updatedTotalTokenUsed = JSON.parse(
